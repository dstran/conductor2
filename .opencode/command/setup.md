...
If `conductor/workflow.md` is missing, explain that the workflow defines the binding "rules of the game" (test enforcement by task type, phase checkpoints, commit strategy) that `/conductor/implement` and `/conductor/review` follow, then write this exact content:
