...
the tracks registry in the track's worktree (`conductor/tracks.md`) — optionally followed by a phase or task
...
Resolve it in the tracks registry in the track's worktree (`conductor/tracks.md`) and the track's `plan.md`. Confirm
with a Yes/No question ("Revert <track/phase/task description>?").

**Path B — nothing was given:**
Scan the tracks registry in the track's worktree (`conductor/tracks.md`) and every listed track's `plan.md`. Prioritize
...
- If the whole track was reverted, also remove the awaiting-review note
  from the tracks registry in the track's worktree (`conductor/tracks.md`) if present, and reset the track's registry
  checkbox to `[ ]`.
